module.exports = function renderList(data) {
  return `<body>
    <div>
    <form action="/links/" method="POST" style="display:flex; align-items:center; flex-direction:column; text-align:center;" >
    ...
    ${data.map((el) => {
      return `
      <div>
      <form action="/links/put/${el.id}" method="POST">
      ...
      <form action="/links/delete/${el.id}" method="POST">
